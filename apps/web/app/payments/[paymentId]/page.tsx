...
export default function PaymentDetailsPage() {
  ...
  return (
    <PageShell title="Payment details">
      ...
      >
        <ArrowLeft /> Payment history
      </Link>
      <Card className="items-center p-7 text-center">
        <div className="flex size-16 items-center justify-center rounded-full bg-accent">
          <Icon className="size-8 text-primary" />
        </div>
        <p className="mt-1 text-sm text-muted-foreground">{payment.type}</p>
        <p className="text-3xl font-bold">{formatNaira(payment.amount)}</p>
      ...
      <Card className="p-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              MoneyCircle records contribution payments against your savings
              circle; it does not hold a wallet balance.
            </p>
          </div>
        </div>
      </Card>
